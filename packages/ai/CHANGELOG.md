...
- Raised the global default first-event stream timeout from 100s to 300s (5 minutes) for all providers. Modern reasoning models (Gemini 3.x Pro thinking, o-series, Claude extended thinking, Grok heavy reasoning, GLM) routinely exceed 100s before emitting their first SSE frame through proxy chains and cold starts. The per-provider `GOOGLE_GEMINI_CLI_LAZY_STREAM_LIMITS` override is removed as it is now redundant with the global floor. The steady-state idle watchdog (120s between events) is unchanged and still catches genuinely stalled streams. `PI_STREAM_FIRST_EVENT_TIMEOUT_MS` and `StreamOptions.streamFirstEventTimeoutMs` still override the default.
